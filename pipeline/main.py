...
import os, logging, srt, yaml, json
from .utils import setup_logging, load_config, ensure_ffmpeg, slugify, derive_show_from_filename, read_meta_yaml
...
def build_clip_srt(segments, t0, t1):
    ...
    for s in segments:
        ...
        st = max(t0, s["start"]); en = min(t1, s["end"])
...
def process_one(cfg, video_path: Path):
    ...
    season = meta.get("season", season or 1)
    episode = meta.get("episode", episode or 1)
    ...
    for k, clip in enumerate(clips, 1):
        ...
        srt_path = out_dir / f"{slugify(show)}_{season_ep}_clip{k}.srt"
        with open(srt_path, "w", encoding="utf-8") as f:
            f.write(clip_srt_text)

        out_mp4 = out_dir / f"{slugify(show)}_{season_ep}_clip{k}.mp4"
        render_clip(video_path, srt_path, out_mp4, clip, cfg)
        ...
        if cfg.get("scheduler",{}).get("enabled", False):
            enqueue(qfile, meta_item)
        else:
            from .upload import upload_short
            vid = upload_short(cfg, channel_alias, out_mp4, title, desc, tags, privacy, str(cat), playlist)
            LOG.info("Immediate upload done: %s", vid)
