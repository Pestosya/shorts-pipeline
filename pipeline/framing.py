# pipeline/framing.py
from pathlib import Path
import subprocess, tempfile, os

# Опционально: OpenCV для лиц. Если нет — сработает fallback на грани.
try:
    import cv2, numpy as np
    _HAS_CV2 = True
except Exception:
    cv2 = None  # type: ignore
    np = None   # type: ignore
    _HAS_CV2 = False

def _ffmpeg_grab_frame(src: Path, t: float, out_w: int = 960) -> Path:
    """Извлекает 1 кадр JPEG во временную папку (масштаб до ширины out_w для скорости)."""
    tmp = Path(tempfile.gettempdir()) / f"frame_{abs(hash((str(src), t)))%10**9}.jpg"
    vf = f"scale={out_w}:-2"
    cmd = ["ffmpeg","-y","-hide_banner","-loglevel","error","-ss", f"{t:.3f}","-i", str(src),
           "-frames:v","1","-vf", vf, str(tmp)]
    p = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    if p.returncode != 0 or not tmp.exists():
        raise RuntimeError("ffmpeg frame grab failed:\n" + p.stdout)
    return tmp

def _detect_interest(img_path: Path):
    """
    Возвращает (cx, cy, w, h) интересной области в координатах изображения.
    Приоритет: лица → крупнейший контур по граням → центр.
    Всегда работает с uint8.
    """
    if not _HAS_CV2:
        from PIL import Image
        with Image.open(str(img_path)) as im:
            w, h = im.size
        return (w/2, h/2, w*0.3, h*0.3)

    img = cv2.imread(str(img_path))
    if img is None:
        raise RuntimeError("cv2.imread failed")
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # uint8 [0..255]

    # 1) Лица (Haar)
    faces = []
    try:
        casc_path = getattr(cv2.data, "haarcascades", "")
        for name in ("haarcascade_frontalface_default.xml","haarcascade_profileface.xml"):
            cpath = os.path.join(casc_path, name)
            if os.path.isfile(cpath):
                face_cascade = cv2.CascadeClassifier(cpath)
                det = face_cascade.detectMultiScale(gray, scaleFactor=1.1, minNeighbors=5, minSize=(40, 40))
                for (x,y,w,h) in det:
                    faces.append((x,y,w,h))
    except Exception:
        faces = []

    if faces:
        x0 = min(x for (x,_,w,_) in faces)
        y0 = min(y for (_,y,_,_) in faces)
        x1 = max(x+w for (x,_,w,_) in faces)
        y1 = max(y+h for (_,y,_,h) in faces)
        return ((x0+x1)/2, (y0+y1)/2, (x1-x0), (y1-y0))

    # 2) Крупная «салсиентная» область по граням (Canny + морфологическое закрытие)
    # авто-порог по медиане
    v = np.median(gray)
    lower = int(max(0, 0.66 * v))
    upper = int(min(255, 1.33 * v))
    edges = cv2.Canny(gray, lower, upper)
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
    edges = cv2.morphologyEx(edges, cv2.MORPH_CLOSE, kernel, iterations=1)

    cnts, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if cnts:
        c = max(cnts, key=cv2.contourArea)
        x,y,w,h = cv2.boundingRect(c)
        return (x + w/2, y + h/2, w, h)

    # 3) Центр по умолчанию
    h, w = gray.shape
    return (w/2, h/2, w*0.3, h*0.3)

def suggest_crop(src: Path, start: float, duration: float,
                 src_w: int, src_h: int,
                 target_w: int, target_h: int,
                 samples: int = 10) -> tuple[float,float]:
    """
    Возвращает смещение кропа (x, y) в СКАЛИРОВАННЫХ координатах после
    scale=target_w:target_h:force_original_aspect_ratio=increase.
    Любые ошибки → центр.
    """
    src_ar = src_w / max(1, src_h)
    tgt_ar = target_w / max(1, target_h)

    # размеры после scale (increase)
    if src_ar >= tgt_ar:
        scaled_w = target_h * src_ar
        scaled_h = target_h
        scale = target_h / max(1, src_h)
    else:
        scaled_w = target_w
        scaled_h = target_w / max(1, src_ar)
        scale = target_w / max(1, src_w)

    # Выбор времени по ролику
    n = max(1, samples)
    times = [start + (i + 0.5) * (duration / n) for i in range(n)]

    cxs, cys = [], []
    for t in times:
        fp = None
        try:
            fp = _ffmpeg_grab_frame(src, t)
            cx, cy, _, _ = _detect_interest(fp)
            cxs.append(cx * scale)
            cys.append(cy * scale)
        except Exception:
            continue
        finally:
            if fp and os.path.exists(fp):
                try: os.remove(fp)
                except Exception: pass

    # Если не получилось — возьмём центр
    if not cxs:
        cxs = [scaled_w / 2]
        cys = [scaled_h / 2]

    cx = sum(cxs) / len(cxs)
    cy = sum(cys) / len(cys)

    x = max(0.0, min(cx - target_w/2, max(0.0, scaled_w - target_w)))
    y = max(0.0, min(cy - target_h/2, max(0.0, scaled_h - target_h)))

    # Вернём целые пиксели, чтобы crop не ругался
    return (float(int(x)), float(int(y)))
